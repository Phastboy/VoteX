...
export default function ElectionSettings() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      const response = await fetch(`https://votex-backend.eastasia.cloudapp.azure.com/elections/${electionId}/settings?category=Results`, {
        method: 'GET',
        headers: headers
      });
  ...
  return (
    <div>
      ...
      {settingsData ? (
        <div>
          <h2>Results Category Settings</h2>
          <p>Setting 1: {settingsData}</p>
        </div>
      ) : (
